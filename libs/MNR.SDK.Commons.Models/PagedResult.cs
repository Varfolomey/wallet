...
public class PagedResult<T> : IResult<T>
{
    ...
    [DataMember(Order = 8)] public string Url { get; set; }
    [DataMember(Order = 9)] public Severity Severity { get; set; }
    ...
    public static PagedResult<T> Code(string message, int code)
    {
        return New(message, code);
    }
